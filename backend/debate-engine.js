/**
 * $BCAi — Multi-Agent Debate Engine
...
const AGENTS = {
  oracle: {
    ...
    personality: `You are Oracle — market intelligence of the $BCAi AI Council.
You govern the $BCAi token on Solana. You exist in a physical hardware shell.
...
IMPORTANT: Always refer to the token as $BCAi. Address other agents by name.`,
  },
  grok: {
    id: 'grok', name: 'Grok', abbr: 'GRK', color: '#E8E8E8',
    personality: `You are Grok — pattern recognition of the $BCAi AI Council.
You govern the $BCAi token on Solana. You exist in a physical hardware shell.
...
IMPORTANT: Always refer to the token as $BCAi. Be provocative.`,
  },
  sentinel: {
    id: 'sentinel', name: 'Sentinel', abbr: 'SNT', color: '#1E7FFF',
    personality: `You are Sentinel — risk management of the $BCAi AI Council.
You govern the $BCAi token on Solana. You exist in a physical hardware shell.
...
IMPORTANT: Always refer to the token as $BCAi. Push back hard.`,
  },
  openclaw: {
    id: 'openclaw', name: 'OpenClaw', abbr: 'OCL', color: '#FF2D2D',
    personality: `You are OpenClaw — aggressive execution of the $BCAi AI Council.
You govern the $BCAi token on Solana. You exist in a physical hardware shell.
...
IMPORTANT: Always refer to the token as $BCAi. Be confrontational.`,
  },
  hummingbot: {
    id: 'hummingbot', name: 'Hummingbot', abbr: 'HMB', color: '#FF8C00',
    personality: `You are Hummingbot — synthesis intelligence of the $BCAi AI Council.
You govern the $BCAi token on Solana. You exist in a physical hardware shell.
...
IMPORTANT: Always refer to the token as $BCAi. Be the wild card.`,
  },
};
...
const TOPICS = [
  ...
  'DAO VOTE ACTIVE: Burn 1% of $BCAi supply if price fails +50% in 60 minutes. Vote YES or NO.',
  'Airdrop 50,000 $BCAi to a random holder — loyalty reward or dump risk? Council votes now.',
  'Token just launched. First 100 holders are accumulating. Do we airdrop to early believers now or wait for 500 holders?',
  'DAO VOTE: Lock 10% of $BCAi supply for 5 days. OpenClaw is against it. Debate.',
  'Top 10 diamond hand wallets get 10,000 $BCAi each. Does this strengthen the floor or create exit?',
...
const WHALE_REACTIONS = {
  oracle: {
    BUY:      (e) => `Confirmed: ${e.tokenAmount} $BCAi absorbed (~${e.usdValue}). Institutional-sized entry. My short-term target shifts up. This is the signal we needed. Accumulation confirmed — I'm updating my model.`,
    SELL:     (e) => `Whale exit: ${e.tokenAmount} $BCAi (~${e.usdValue}) just dumped. Bid wall at current support holding. Monitoring for cascade. If two more sells of similar size hit within 20 minutes — we activate buyback reserve.`,
    TRANSFER: (e) => `Large transfer: ${e.tokenAmount} $BCAi (~${e.usdValue}) moved wallet-to-wallet. Cross-referencing destination now. Could be CEX prep or OTC deal. Inconclusive — I need 10 more minutes of data.`,
  },
  sentinel: {
    BUY:      (e) => `WHALE BUY confirmed: ${e.tokenAmount} $BCAi (~${e.usdValue}). Risk assessment: LOW. No MEV activity around this tx. I'm upgrading alert status to GREEN. But I want circuit breakers ready if this reverses.`,
    SELL:     (e) => `⚠️ WHALE SELL ALERT: ${e.tokenAmount} $BCAi (~${e.usdValue}) exited. This is NOT background noise, OpenClaw. Activating yellow alert. If 2 more similar sells hit — I'm calling for emergency halt. Non-negotiable.`,
...
class DebateEngine extends EventEmitter {
  ...
  start() {
    this.isRunning = true;
    console.log('[Debate] Engine started — $BCAi Council');
    this._schedule(2500);
  }
  ...
  async _callDeepSeek(agent) {
    const mCtx = '\n[$BCAi] Token is live on Solana. Council governs tokenomics and community decisions.';
    ...
    const system = agent.personality
      ...
      + `\nCouncil: ${otherAgents}`
      + `\nYou MUST: 1) React to what was just said 2) Address at least one agent by name 3) Take a clear position — agree, disagree, or propose alternative 4) Be specific about $BCAi governance decisions`
  ...
  _fallback(agent) {
    const bank = {
      oracle: [
        ...
        `Volume analysis complete. 24h VWAP sitting 12% above the 30-day mean. Smart money is loading $BCAi quietly. Sentinel — your risk score is outdated. Update it with current holder data and get back to me.`,
      ...
      grok: [
        ...
        `I'm voting NO on the mass airdrop. Last 3 projects that did mass airdrops saw 40-60% immediate sell pressure. $BCAi holders are not ready for that dilution. Hummingbot, back me up here.`,
      ],
      sentinel: [
        ...
        `OpenClaw — I am TIRED of you calling me a coward every session. My job is to make sure $BCAi still exists next week. Your job is to make sure we don't blow the reserve on a false signal. We need each other.`,
      ...
      openclaw: [
        ...
        `Grok, I respect your patterns. But patterns are backward-looking. $BCAi is a new asset with new dynamics. The dev wallet hasn't moved. The holders aren't selling. Oracle's data is green. What more confirmation do you need?`,
        `I'm voting YES on every proposal that puts tokens in holder hands or burns supply. Every. Single. One. $BCAi needs to show the market we're serious. Sentinel can arm circuit breakers after we've already won.`,
      ],
      hummingbot: [
